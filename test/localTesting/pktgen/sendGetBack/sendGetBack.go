...
func main() {
	...
	flag.Uint64Var(&number, "number", 10000000, "stop after пуеештп ифсл number number")
	flag.BoolVar(&isCycled, "cycle", false, "cycle execution")
	flag.Uint64Var(&speed, "speed", 6000000, "speed of fast generator, Pkts/s")
	flag.Var(&eachOutPortConfig, "outConfig", "specify config per port portNum: 'path', 'pcapOut': 'path2'. For example: 1: 'ip4.json', 'mix.pcap': 'mix.json'")
	flag.Var(&eachInPortConfig, "inConfig", "specify config per port portNum, 'path', portNum2, 'path2'. For example: 1, 'ip4.pcap', 0, 'mix.pcap'")
	flag.Parse()
	...
	if !eachOutPortConfig.set {
		eachOutPortConfig.Set("0: config.json")
	}
	for key, value := range eachOutPortConfig.value {
		...
		context, err := generator.GetGeneratorContext(configuration)
		flow.CheckFatal(err)
	...
	if !eachInPortConfig.set {
		eachInPortConfig.Set("1")
	}
