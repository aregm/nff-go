...
type ARPHdr struct {
	...
	SHA       common.MACAddress         // Sender hardware address (sender MAC address)
	...
	THA common.MACAddress         // Target hardware address (target MAC address)
...
func InitARPRequestPacket(packet *Packet, SHA common.MACAddress, SPA, TPA common.IPv4Address) bool {
...
// SHA.
func InitARPReplyPacket(packet *Packet, SHA, THA common.MACAddress, SPA, TPA common.IPv4Address) bool {
	if !initARPCommonData(packet) {
		common.LogWarning(common.Debug, "InitARPRequestPacket: failed to fill common data")
		return false
	}
...
// FF:FF:FF:FF:FF:FF (broadcast) and source address is set to SHA.
func InitGARPAnnouncementRequestPacket(packet *Packet, SHA common.MACAddress, SPA common.IPv4Address) bool {
	if !initARPCommonData(packet) {
		common.LogWarning(common.Debug, "InitARPRequestPacket: failed to fill common data")
		return false
	}
	...
	arp.SPA = common.IPv4ToBytes(SPA)
	arp.THA = [common.EtherAddrLen]uint8{0xff, 0xff, 0xff, 0xff, 0xff, 0xff}
	arp.TPA = common.IPv4ToBytes(SPA)
	return true
}
...
// to SHA.
func InitGARPAnnouncementReplyPacket(packet *Packet, SHA common.MACAddress, SPA common.IPv4Address) bool {
	if !initARPCommonData(packet) {
		common.LogWarning(common.Debug, "InitARPRequestPacket: failed to fill common data")
		return false
	}
