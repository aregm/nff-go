...
<br>
/<a href="/telemetry">telemetry</a> for all node names and their counters which include
received, send, processed, lost and dropped packets. Using
/telemetry/name returns information about individual node.<br>
/<a href="/json/telemetry">json/telemetry</a> for JSON data structure enumerating all
nodes that have statistics or /json/telemetry/name fo JSON data structure with statistics
of individual node.
</body></html>`
...
<html lang="en">
    <head>
        ...
        var intervalTimer;
        var droppedPackets = {{if .DoDropped}}true{{else}}false{{end}};
                ...
                if (delta_button.checked) {
                    if (prevDataAvailable) {
                        if (droppedPackets) {
                            ...
                            pkts_data.addRow(["" + (seconds_counter).toFixed(1), prevPacketsProcessed]);
                        }
                        ...
                        if (droppedPackets) {
                            ...
                            pkts_data.addRow(["" + (seconds_counter).toFixed(1), undefined]);
                        }
                    ...
                    if (droppedPackets) {
                        ...
                        pkts_data.addRow(["" + (seconds_counter).toFixed(1), dataObj.PacketsProcessed]);
                    }
        ...
        function drawChart()
        {
            pkts_object = new google.visualization.LineChart(pkts_chart);
            bytes_object = new google.visualization.LineChart(bytes_chart);
...
{{if .DoDropped}}            </tr><tr>
                ...
                <td><div id="pktsDroppedDelta">no data</div></td>{{end}}{{/* .DoDropped */}}
            </tr><tr>
...
func handleTelemetryStats(w http.ResponseWriter, r *http.Request) {
	data := struct {
		StatsName string
		Values    map[string]*common.NodeTelemetry
	}{
		StatsName: "telemetry",
		Values:    telemetry,
	}
	err := statsSummaryTemplate.Execute(w, &data)
	if err != nil {
		fmt.Println("Error in telemetry summary stats", err)
	}
}

func handleTelemetryStatsNode(w http.ResponseWriter, r *http.Request) {
	url := strings.Split(r.URL.Path, "/")
	sendNodeVisualization(w, r, "telemetry", url[2], false)
}
...
func handleJSONTelemetryStats(w http.ResponseWriter, r *http.Request) {
	enc := json.NewEncoder(w)

	w.Header().Set("Content-Type", "application/json")
	names := make([]string, len(telemetry))
	index := 0
	for keys := range telemetry {
		names[index] = keys
		index++
	}
	enc.Encode(names)
...
func handleJSONTelemetryStatsNode(w http.ResponseWriter, r *http.Request) {
	url := strings.Split(r.URL.Path, "/")
	enc := json.NewEncoder(w)

	stats, ok := telemetry[url[3]]
	if !ok {
		http.Error(w, "Bad node name: "+url[3], http.StatusBadRequest)
		return
	}
...
func initCounters(addr *net.TCPAddr) error {
	...
	http.HandleFunc("/telemetry/", handleTelemetryStatsNode)
	http.HandleFunc("/telemetry", handleTelemetryStats)
	http.HandleFunc("/json/rxtx/", handleJSONRXTXStatsNode)
	http.HandleFunc("/json/rxtx", handleJSONRXTXStats)
	http.HandleFunc("/json/telemetry/", handleJSONTelemetryStatsNode)
	http.HandleFunc("/json/telemetry", handleJSONTelemetryStats)
